...
export function classifyConnection(effectiveType?: string, downlink?: number): ConnectionTier {
...
export function classify(signals: RawSignals): DeviceTiers {
  return {
    ...
    connection: classifyConnection(signals.connection?.effectiveType, signals.connection?.downlink),
  };
}
