...
app.get('/', (req, reply) => {
  ...
  const forceParam = (req.query as Record<string, string | undefined>).force;
...
<head>
  ...
  <title>DeviceRouter \u2014 Fastify Adaptive Demo</title>
...
${full ? '    <svg style="display:block;width:100%;position:absolute;bottom:-1px;left:0" viewBox="0 0 1440 50" preserveAspectRatio="none"><path d="M0,25 C360,50 720,0 1080,25 L1440,25 L1440,50 L0,50Z" fill="#f8fafc"/></svg>' : ''}
  ...
  <div class="features">
    <div class="card">
      ...
      <p>A 762-byte probe collects CPU, memory, and network signals via browser APIs \u2014 no user-agent sniffing.</p>
    </div>
    <div class="card">
      <span class="card-icon">${
        full
      ...
      <p>Devices are classified into CPU, memory, and connection tiers. Thresholds are fully customizable.</p>
    </div>
    <div class="card">
      <span class="card-icon">${
        full
